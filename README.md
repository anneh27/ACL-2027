...
Week 4 replaces the Claude-as-subject runs with a real, independent model. `.env` holds the OpenAI key (gitignored) and `OPENAI_MODEL=gpt-5.6-sol`. **One command regenerates the MGSM results:** `./run_week4_mgsm.sh`.

- `src/fetch_mgsm_trilingual.py` pulls the official English/Spanish/Japanese MGSM test sets; `src/run_mgsm_trilingual.py` makes 150 real calls (50 shared questions x 3 languages); `src/mgsm_scoring.py` holds the tested `normalize_answer`; `src/mgsm_error_diagnosis.py` re-samples and back-translates every wrong answer; `src/verify_scoring_dispute.py` recomputes the Cherokee label scores from the original log.
- Results: English 50/50, Spanish 48/50, Japanese 48/50. All 4 errors trace to differences in the official translations, not to model failure in the target language, and the model runs at its default temperature because it rejects `temperature=0`. Full facts, interpretation, and limitations: `results/week4_audit.md`. One-page summary of the whole week: `results/week4_one_page_summary.md`.
- P1 (Cherokee pilot v2): the Week 2/3 B/C answers were hand-transcribed from Claude subagent replies, so they were not verifiable originals. The original 20 items were re-run through the real model (`src/run_cherokee_pilot_v2.py`, 100 calls, every raw output in `results/cherokee_pilot_v2_log.jsonl`): A 20/20, B 18/20, C (back-translate, then answer) 15/20, D 19/20. Several errors are unstable across re-samples (`src/cherokee_pilot_v2_stability.py`), the D miss on `spat_04` is a dataset gap (no transitivity axiom), and most correct answers rest on the logical skeleton surviving while entities and predicates change. Error types and fidelity ratings are Claude's judgment and need verification. Details: `results/cherokee_pilot_v2_audit.md`.
- Still open: the 20 harder Week 3 logic items have not been run through the real model, and no native Cherokee speaker has reviewed any translation.
